...
| **RoundPhase** | `SUBMIT` → (`SUBMIT_LIVE`) → optional `VOTE` → `LOCKED`/`REVEAL_RESPONSES` → `REVEAL_RESULTS`. `VOTE` is the *current* round's own best-answer/deception voting — distinct from Best Answer mode below. |
