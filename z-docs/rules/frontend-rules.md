...
4. **Lint clean** — the fast lint (plain oxlint + Stylelint) must report zero errors before every commit; `scripts/pre-commit` enforces it. The full type-aware lint (`npm run lint:all`) and the typecheck must additionally be clean at the feature gate, `scripts/check-feature.sh`. — see [testing-and-ci](../infrastructure/testing-and-ci.md)
