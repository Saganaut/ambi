...
1. **Icons vs images** — Monochrome UI marks go in `assets/icons/<category>/`; multi-color illustrations, mascots, and logos go in `assets/images/<category>/`. Categories: icons by function (`action`, `charts`, `content`, `interface`, `navigation`, `slide-types`, `social`, `status`, `theme`, `user`); images by kind (`avatars-to-process`, `backgrounds`, `brand`, `card-types`, `landing-page`, `mascots`, `slide-types`). `images/unsorted/` is a triage inbox — never reference it from code.
...
5. **Consume via `?react` only** — `import DeleteIcon from "@assets/icons/action/delete.svg?react"`, then render `<DeleteIcon className={styles.btnIcon} aria-hidden="true" />`. Without the suffix the import is a URL string. Never write inline `<svg>` for new artwork, and don't put SVGs in `public/` unless they must be fetched by URL.
