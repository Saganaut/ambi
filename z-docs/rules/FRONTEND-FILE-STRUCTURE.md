...
├── store/ # Core global network backbone (Global state registry & API setup) location of autogenerated api
│ ├── store.ts # Primary store configuration
│ ├── hooks.ts # Fully typed UseAppSelector and UseAppDispatch hooks
│ └── [global]Slice.ts # Global state slices (e.g. global settings)
