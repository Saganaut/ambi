...
- **Imperative dialogs are promise-based hooks.** Confirms and login prompts resolve a promise rather than taking callbacks — `const ok = await confirm({...}); if (!ok) return;` (`useConfirm`, `useRequireLogin`). This is also why `window.confirm`/`alert`/`prompt` are banned ([styling-rules.md](../styling-rules.md) §7).
