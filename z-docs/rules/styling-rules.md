...
CSS, tokens, and design-system conventions. Full explanation and migration status: [frontend/STYLES.md](../../frontend/STYLES.md).

1. **CSS Modules** — Files end in `*.module.css`; class names are lowerCamelCase (Stylelint enforces). No global styles, except the modifier rules in `tokens.css` (theme classes + `data-*` selectors).
...
4. **Tokens & the variable manifest** — Every value comes from a `tokens.css` token (never hardcode); variantizable components read their style from local CSS vars via the load-bearing `var(--name, fallback)` pattern. — [details](styling/tokens-and-variables.md)
