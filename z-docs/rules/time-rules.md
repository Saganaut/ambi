...
2. **Use `Auditable` for `createdAt` / `updatedAt`** — Extend `cephadex.ambi.model.Auditable` (it declares `@CreatedDate`/`@LastModifiedDate Instant` fields, auto-populated by Spring Data auditing). Never call `setCreatedAt`/`setUpdatedAt` manually, and never re-declare those fields on a subclass.
