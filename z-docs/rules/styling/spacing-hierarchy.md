...
1. **Outer ≥ inner, by at least one ladder step.** A container's gap between children must exceed the gaps *inside* those children, and its padding should be ≥ its own gap.
...
`frontend/stylelint.config.mjs` bans `var(--space-*)` in `padding*`, `margin*`, and `gap` (error severity). Positioned offsets, `scroll-margin`, and token definitions are outside the rule's property set by design; a ratified one-off on a flagged property carries a `stylelint-disable-next-line` with a reason.

Reference example: `frontend/src/features/deck/views/MyDecksPage/MyDecksPage.module.css`.
