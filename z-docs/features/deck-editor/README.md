...
- Toolbar capabilities today: bold / underline / strike / link (inline URL editor, no `window.prompt`) / color (the DS `ColorPickerNew`, see [Color pickers](#color-pickers) below) / 4-step font size. Extensions in use: `@tiptap/starter-kit`, `@tiptap/extension-text-style` (TextStyle + Color + FontSize), `@tiptap/extensions` (Placeholder).
...
Every DS color picker in the editor — the rich-text toolbar's text color, `EditSlidePanel`'s per-slide background color, and the option/item "custom color" modal (`SlideContent/_shared/OptionMenu/useCustomColorModal.tsx`) — is `ColorPickerNew` (`frontend/src/shared/components/Forms/Input/ColorPicker/ColorPickerNew/`), and all three share one app-wide "recently used" list.

- `useRecentColors` (`frontend/src/shared/hooks/useRecentColors.ts`) owns that list; `ColorPickerNew`'s `recentlyUsedColorSwatch` prop is caller-owned, so each consumer reads `useRecentColors()` and passes it in, then calls `addRecentColor(color)` from its own `onChange`/Apply handler.
...
- Values read back from storage are validated with the `isColorValue` type guard (`ColorPickerNew/colorConversion.ts`); corrupt JSON or non-color entries are silently discarded, degrading to an empty list. A storage write failure (private browsing, quota exceeded) degrades to in-memory-only for the rest of the session — the picker keeps working, it just won't persist.
