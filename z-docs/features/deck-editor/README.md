...
- `PlacementItemRow` — the presentational item row (colored index pill, label field + popover menu, image thumbnail, optional trailing meta slot), plus its dnd wrapper `SortablePlacementRow` (reorderable rows — Axis and Grid). A wrapper rather than a prop because the rules of hooks forbid a component turning `useSortable` on and off.
