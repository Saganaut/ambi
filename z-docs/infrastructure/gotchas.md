...
- WebSocket support is live for live sessions: `session/transport/WebSocketConfig` registers a `/ws` STOMP endpoint with a `/topic` broker, and the Redis-backed relay fans live-session events out to it. See [live-session-flow](../live-session-flow.md) and [live-session-open-decisions](../live-session-open-decisions.md).
