 ...
 * @param allocationTargets     the Allocation round's authored per-option point key, disclosed only once the round is revealing results (phase {@code REVEAL_RESULTS}); {@code null} otherwise, so a late joiner rehydrates the key reveal exactly as the {@link com.cephadex.ambi.session.event.ResultsRevealed} delta carries it
