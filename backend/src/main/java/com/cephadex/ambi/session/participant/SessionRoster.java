 ...
 * and writes what it found back. The durable side carries both ends of the
 * membership window — an {@code admittedAt} that only a landed admit stamps and a
 * {@code leftAt} that only an explicit leave does — so a rehydrate can resurrect
 * neither someone who left nor someone who was never admitted. The healing is
 * add-only — no heal ever prunes, only an explicit {@link #remove} or
 * {@link #clear} does — and the admit script's {@code PEXPIRE} refreshes the TTL
 * on every join, so a phantom member that slips past a join's rollback stays for
 * the session's life.
 */
@Component
public class SessionRoster {
     ...
     * check. An id the set already carries has not thereby been given a seat — a
     * heal can only have put it there — so the discount restores the count the cap
     * should see; a retry after a {@code SADD} that landed finds them occupying a
     * counted seat, so the discount cancels it and the admit stays idempotent.
     * Waiving the check outright would instead let any pre-seeded id join a session
     * already at its limit. Belt-and-braces since the roster seed became
     * admitted-only (a mid-flight joiner is no longer seeded at all), and kept for
     * the cases that survive it: a member the set lost and a heal put back, or an
     * admit retried after its reply was lost.
     */
    ...
    /**
     * The session's admitted members, in join order, straight from the durable
     * documents — the Redis set holds ids only, and order is load-bearing for the
     * snapshot.
     *
     * <p>A document without {@code admittedAt} is a join still in flight, and every
     * consumer wants it left out: the rehydrate seed must not hand a seat to
     * someone the admit may refuse, and scoring must not touch a document a
     * concurrent rollback is about to delete ({@code RoundResultProjector.persist}
     * saves the roster back, which would resurrect it).
     */
    public List<Participant> participants(String sessionId) {
        return participants.findBySessionIdAndLeftAtIsNullAndAdmittedAtNotNullOrderByJoinedAtAsc(sessionId);
    }
     ...
     *
     * <p>The rehydrate that precedes the cap check still excludes
     * {@code participantId}: a heal seeds the members that hold seats, and this one
     * has not been admitted yet. Belt-and-braces now that the seed reads admitted
     * documents only — the joiner's document carries no admission marker until this
     * returns, so it could not be seeded anyway — but the exclusion costs nothing
     * and keeps the invariant local. The cap check itself does not depend on it:
     * the script discounts a joiner the set already carries either way.
     *
     ...
     *
     * <p>Only an admitted document answers {@code true} here, which leaves one
     * accepted race: a key lost <em>after</em> a joiner's admit landed but before
     * their marker was stamped rehydrates without them, and a later membership check
     * — finding the marker by then — heals them back in without consulting the cap,
     * so a session can transiently sit at cap+1. It needs key loss, a concurrent
     * rehydrate inside a single Mongo round-trip, and the backfilled seat to be
     * asked for later; it is bounded, transient, and errs towards keeping a genuine
     * member, the same class as the accepted orphan legs of a failed join rollback.
     * The mitigation — a cap-aware heal — would lock genuine members out of their
     * own session, which is the direction that must never happen.
     */
    public boolean contains(String sessionId, String participantId) {
        ...
        if (!participants.existsByParticipantIdAndSessionIdAndLeftAtIsNullAndAdmittedAtNotNull(
     ...
     *
     * <p>The seed is <strong>admitted-only</strong> ({@link #participants} filters
     * on the durable admission marker), which is what makes a mid-flight joiner
     * invisible to it: a burst of joins against a cold key can no longer cross-seed
     * each other's speculative documents and refuse joiners that would have fit.
     *
     ...
     *
     * @param excludeParticipantId a joiner who has not been admitted yet, so the cap
     *                             check cannot count them twice; belt-and-braces
     *                             against the admitted-only seed. {@code null} to
     *                             seed the roster whole
