 ...
 *
 * <ol>
 *   <li>{@link RoundEvaluator#evaluate} — derive the per-participant facts once;</li>
 ...
 * </ol>
 *
 * <p>It mutates the passed-in {@link Participant}s' scores but persists nothing:
...
public final class RoundScorer {
     ...
     *
     * @param sid              the live session
     * @param slide            the slide that was open (from the session's deck snapshot)
     * @param answers          every answer submitted this round
     * @param participantsById the round's participants keyed by {@code participantId},
    ...
    public static RoundResult score(
        ...
        for (AnswerEvaluation eval : evaluations) {
            Participant participant = participantsById.get(eval.participantId().value());
            ...
            int pointsAwarded = participant == null ? 0 : participant.awardPoints(
                    eval.correct(),
                    eval.bestAnswer(),
                    eval.fastestCorrect(),
                    eval.deceivedCount(),
        ...
        return RoundResult.compute(sid, slide, outcomes, closedAt);
    }
}
