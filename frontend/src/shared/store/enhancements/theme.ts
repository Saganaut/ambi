...
import {
  Ambi,
  type DeleteThemeApiArg,
  type ListThemesForOrgApiArg,
  type ThemeResponse,
  type UpdateThemeApiArg,
} from "../AmbiApi";
...
const writeThemeIntoLists = (
  ...
  dispatch(
    Ambi.util.updateQueryData("listMyThemes", undefined, (draft) => {
      upsert(draft, theme);
    }),
  );
};
    ...
    updateTheme: {
      onQueryStarted: async (
        ...
        const getPatch = dispatch(
          Ambi.util.updateQueryData("getTheme", { id: arg.id }, (draft) => {
        ...
        try {
          const { data } = await queryFulfilled;
          dispatch(
            Ambi.util.updateQueryData("getTheme", { id: arg.id }, () => data),
          );
          writeThemeIntoLists(dispatch, data);
        } catch {
          getPatch.undo();
    ...
    deleteTheme: {
      onQueryStarted: async (arg: DeleteThemeApiArg, api: CacheSyncApi) => {
        ...
        for (const entry of Object.values(queries)) {
          ...
          patch(
            Ambi.util.updateQueryData("listThemesForOrg", queryArg, (draft) =>
              draft.filter((t) => t.id !== arg.id),
            ),
          );
        }
