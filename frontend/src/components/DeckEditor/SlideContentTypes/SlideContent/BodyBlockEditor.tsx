...
import type { BodyBlock } from "@/store/slideBlockTypes";
...
const BodyBlockEditor = ({ block, onUpdate, onFlush }: BodyBlockEditorProps) => {
  return (
    <RichTextInput
      label='Body'
      ...
      onChange={(html) => { onUpdate({ ...block, richBody: html }, "schedule"); }}
      onBlur={onFlush}
    />
  );
};
