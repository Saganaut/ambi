...
const AnswerPanel = ({ deckId, slideId }: deckAndSlideIdProps) => {
  ...
  const handleChange = (patch: Partial<AnswerSettings>, { immediate }: { immediate: boolean }) => {
  ...
  const toggle = (key: keyof AnswerSettings) => (e: React.ChangeEvent<HTMLInputElement>) => {
    handleChange({ [key]: e.currentTarget.checked }, { immediate: true });
  };

  const number = (key: keyof AnswerSettings) => (next: number) => {
    ...
    <div className={slidePanel.panel}>
      ...
      {isScorable && (
        <section className={`${slidePanel.section} ${styles.sectionDivider}`}>
          ...
          >
            <Btn onClick={applyToDeck}>Apply to all slides</Btn>
          </Tooltip>
        </section>
      )}
      ...
      <section
        className={[slidePanel.section, isScorable ? styles.sectionDivider : ""]
          ...
          .join(" ")}
      >
        {slide && <PerKindSection contentType={slide.content.contentType} />}
      </section>
