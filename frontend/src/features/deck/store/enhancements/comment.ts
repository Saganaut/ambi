...
const patchSlideThreads = (
  ...
  for (const entry of Object.values(queries)) {
    ...
    const queryArg = (entry.originalArgs ??
      {}) as ListSlideCommentThreadsApiArg;
    if (queryArg.deckId !== deckId || queryArg.slideId !== slideId) continue;
    api.dispatch(
      commentApi.util.updateQueryData(
        "listSlideCommentThreads",
        queryArg,
        (draft) => {
...
>() => ({
  onQueryStarted: async (arg: Arg, api: CacheSyncApi) => {
    try {
      const { data } = (await api.queryFulfilled) as {
        data: CommentThreadResponse;
      };
...
commentApi.enhanceEndpoints({
  endpoints: {
    createCommentThread: {
      onQueryStarted: async (
        arg: CreateCommentThreadApiArg,
        api: CacheSyncApi,
      ) => {
        let data: CommentThreadResponse;
        try {
          ({ data } = (await api.queryFulfilled) as {
            data: CommentThreadResponse;
          });
        } catch {
