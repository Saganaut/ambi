...
const optimisticDeleteDeck = async (arg: { id: string }, api: CacheSyncApi) => {
  ...
  for (const entry of Object.values(queries)) {
    ...
    if (entry.endpointName === "listDecksForOrg") {
      ...
      patch(
        deckApi.util.updateQueryData("listDecksForOrg", queryArg, (draft) =>
          draft.filter((deck) => deck.id !== arg.id),
        ),
      );
      ...
      patch(
        deckApi.util.updateQueryData("listPublicDecks", queryArg, (draft) => {
...
deckApi.enhanceEndpoints({
  endpoints: {
    ...
    setDeckAudienceSettings:
      reconcilingDeckMutation<SetDeckAudienceSettingsApiArg>((draft, arg) => {
        draft.settings = {
          ...draft.settings,
    ...
    setDeckBackgroundImage:
      reconcilingDeckMutation<SetDeckBackgroundImageApiArg>((draft, arg) => {
