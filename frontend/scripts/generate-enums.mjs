 ...
 */
const ENUMS = [
  { name: "PublishStatus", feature: "deck", prop: ["DeckResponse", "publishStatus"] },
  { name: "DeckVisibility", feature: "deck", prop: ["DeckResponse", "visibility"] },
  { name: "SlideType", feature: "deck", discriminatorOf: "SlideContent", emitList: "SLIDE_TYPE_LIST" },
  { name: "Difficulty", feature: "deck", prop: ["SlideResponse", "difficulty"] },
  { name: "ScoreMode", feature: "deck", prop: ["NumberContent", "scoreMode"] },
  { name: "UserLevel", feature: "auth", prop: ["RegisteredMe", "userLevel"] },
  { name: "MembershipTier", feature: "auth", prop: ["RegisteredMe", "effectiveTier"] },
  { name: "MembershipStatus", feature: "auth", prop: ["RegisteredMe", "membershipStatus"] },
];
...
function resolveValues(entry, schemas) {
  ...
  if (entry.prop) {
    ...
    if (!Array.isArray(raw)) {
      throw new Error(`Enum "${entry.name}": ${schemaName}.${fieldName} has no \`enum\` array.`);
    }
    ...
    throw new Error(`Enum "${entry.name}": registry entry needs \`prop\` or \`discriminatorOf\`.`);
  }
...
async function main() {
  ...
  for (const [feature, blocks] of Object.entries(byFeature)) {
    const outPath = resolve(FEATURES_DIR, feature, "store", `${feature}Enums.gen.ts`);
    const header =
